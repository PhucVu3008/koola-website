...
import { Save, X, Loader2, AlertCircle, Eye, ShieldAlert } from 'lucide-react';
...
interface UserFormProps {
  ...
  locale: string;
  mode: 'create' | 'edit';
  /** When true, all inputs are disabled and save/delete actions are hidden. */
  viewOnly?: boolean;
}
...
export function UserForm({ userId, locale, mode, viewOnly = false }: UserFormProps) {
  ...
  /** CSS class for inputs — disabled + styled differently in view-only mode */
  const inputCls = `w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 ${
    viewOnly
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          {viewOnly && <Eye className="w-6 h-6 text-gray-500" />}
          {viewOnly
            ? (locale === 'vi' ? 'Chi tiết người dùng' : 'User Details')
            : (mode === 'create' ? t.admin.users.createTitle : t.admin.users.editTitle)}
        </h1>
        <p className="text-sm text-gray-600 mt-1">
          {viewOnly
            ? (locale === 'vi' ? 'Thông tin tài khoản người dùng (chỉ xem)' : 'User account information — read only')
            : (mode === 'create'
      ...
      {viewOnly && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6 flex items-start gap-3">
          ...
          <p className="text-sm font-medium text-amber-900">
            {locale === 'vi'
              ? 'Bạn không có quyền chỉnh sửa thông tin người dùng. Liên hệ Admin để thực hiện thay đổi.'
              : 'You do not have permission to edit user information. Contact an Admin to make changes.'}
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t.admin.users.form.email} {!viewOnly && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="email"
                ...
                {/* Password — hidden in view-only mode */}
                {!viewOnly && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <input
                      ...
                      placeholder={mode === 'edit'
                        ? (locale === 'vi' ? 'Để trống nếu không đổi' : 'Leave blank to keep current')
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t.admin.users.form.fullName} {!viewOnly && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="text"
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">
                {locale === 'vi' ? 'Vai Trò & Quyền Hạn' : 'Roles & Permissions'}
              ...
              <div className="space-y-3">
                {roles.map((role) => (
                  <div
                    ...
                    onClick={() => !viewOnly && handleRoleToggle(role.id)}
                  >
                    <input
                      type="checkbox"
                      checked={formData.role_ids.includes(role.id)}
                      onChange={() => !viewOnly && handleRoleToggle(role.id)}
                      className="mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      disabled={viewOnly}
                    />
                    <div className="ml-3 flex-1">
                      <label className={`block text-sm font-medium text-gray-900 capitalize ${!viewOnly && 'cursor-pointer'}`}>
                        {role.name}
                      </label>
                      {role.description && (
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
            ...
            >
              <X className="w-4 h-4" />
              {viewOnly ? (locale === 'vi' ? 'Quay lại' : 'Back') : (locale === 'vi' ? 'Hủy' : 'Cancel')}
            </button>
            {!viewOnly && (
              <button
                type="submit"
